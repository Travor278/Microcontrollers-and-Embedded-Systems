...
{
  SD_Error errorstatus = SD_OK;
  printf("TF_LOW,state=sd_read_block_begin,addr=%lu,block=%u,card=%u\r\n", (unsigned long)ReadAddr, (unsigned int)BlockSize, (unsigned int)CardType);
#if defined (SD_POLLING_MODE) 
  uint32_t count = 0, poll_timeout = 0x00FFFFFF, *tempbuff = (uint32_t *)readbuff;
#endif
  ...
  printf("TF_LOW,state=sd_read_block_cmd16,result=%u\r\n", (unsigned int)errorstatus);
 ...
 /*********************************************************************************/
  ...
  printf("TF_LOW,state=sd_read_block_data_config\r\n");
  ...
  printf("TF_LOW,state=sd_read_block_cmd17,result=%u\r\n", (unsigned int)errorstatus);
  ...
  /*!< Polling mode: drain FIFO as soon as any word is available. */
  count = (uint32_t)BlockSize / 4U;
  while ((count > 0U) && (poll_timeout > 0U))
  {
    poll_timeout--;
    if ((SDIO->STA & (SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_STBITERR)) != 0U)
    {
      break;
    }
  ...
  if ((poll_timeout == 0U) || (count > 0U) || (SDIO_GetFlagStatus(SDIO_FLAG_DTIMEOUT) != RESET))
  {
    printf("TF_LOW,state=sd_read_poll_timeout,left=%lu,sta=0x%08lx,timeout=%lu\r\n", (unsigned long)count, (unsigned long)SDIO->STA, (unsigned long)poll_timeout);
  ...
  SDIO_ClearFlag(SDIO_STATIC_FLAGS);
    ...
    printf("TF_LOW,state=sd_read_block_dma_ready\r\n");
#endif
...
SD_Error SD_WaitReadOperation(void)
{
  ...
  uint32_t timeout = 0x00FFFFFF;
		  //等待dma传输结束
  while ((SD_DMAEndOfTransferStatus() == RESET) && (TransferEnd == 0) && (TransferError == SD_OK) && (timeout > 0U))
  {
    timeout--;
  }
...
SD_Error SD_WriteBlock(uint8_t *writebuff, uint64_t WriteAddr, uint16_t BlockSize)
{
...
#if defined (SD_POLLING_MODE)
  uint32_t bytestransferred = 0, count = 0, restwords = 0, poll_timeout = 0x00FFFFFF;
 ...
 /*********************************************************************************/
  ...
  while (!(SDIO->STA & (SDIO_FLAG_DBCKEND | SDIO_FLAG_TXUNDERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_STBITERR)) && (poll_timeout > 0U))
  {
    poll_timeout--;
  ...
  if ((poll_timeout == 0U) || (SDIO_GetFlagStatus(SDIO_FLAG_DTIMEOUT) != RESET))
  {
    SDIO_ClearFlag(SDIO_FLAG_DTIMEOUT);
    errorstatus = SD_DATA_TIMEOUT;
    return(errorstatus);
...
{
  SD_Error errorstatus = SD_OK;
  uint32_t count = 0, poll_timeout = 0x00FFFFFF;
  ...
  while (!(SDIO->STA &(SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DBCKEND | SDIO_FLAG_STBITERR)) && (poll_timeout > 0U))
  {
    poll_timeout--;
...
static SD_Error IsCardProgramming(uint8_t *pstatus)
{
  ...
  status = SDIO->STA;
  while (!(status & (SDIO_FLAG_CCRCFAIL | SDIO_FLAG_CMDREND | SDIO_FLAG_CTIMEOUT)) && (timeout > 0U))
  {
    timeout--;
    status = SDIO->STA;
  }
	  /*一系列的状态判断*/
  if ((timeout == 0U) || (status & SDIO_FLAG_CTIMEOUT))
  {
    errorstatus = SD_CMD_RSP_TIMEOUT;
    SDIO_ClearFlag(SDIO_FLAG_CTIMEOUT);
    return(errorstatus);
...
{ 
  ...
  uint32_t index = 0, poll_timeout = 0x00FFFFFF;
  SD_Error errorstatus = SD_OK;
