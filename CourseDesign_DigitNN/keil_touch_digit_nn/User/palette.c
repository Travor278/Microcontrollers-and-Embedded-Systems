...
static void Draw_Recognize_Button(void *btn);
...
static void Command_Recognize_Digit(void *btn);
...
void Touch_Button_Init(void)
{
  ...
  button[15].draw_btn = Draw_Recognize_Button ;
  button[15].btn_command = Command_Recognize_Digit ;
}
...
//  RA8875_SetActiveWindow(PALETTE_START_X,PALETTE_START_Y,PALETTE_END_X,PALETTE_END_Y);
  ...
  if(x>PALETTE_START_X && pre_x>PALETTE_START_X )
  {
    if (brush->shape != RUBBER) {
      DigitRecognition_AddTouchPoint(pre_x, pre_y);
      DigitRecognition_AddTouchPoint(x, y);
    }
...
/**
* @brief  Draw_Recognize_Button 绘制识别按钮
* @param  btn Touch_Button 类型的按键参数
* @retval 无
*/
static void Draw_Recognize_Button(void *btn)
{
  ...
  if(ptr->touch_flag == 0)
  {
    ...
    ILI9341_DrawRectangle(ptr->start_x,
                          ...
                          ptr->end_x - ptr->start_x,
                          ptr->end_y - ptr->start_y,1);
    LCD_SetColors(CL_WHITE,CL_BLUE4);
  }
  else
  {
    LCD_SetColors(CL_WHITE,CL_WHITE);
  ...
  LCD_SetFont(&Font8x16);
  ILI9341_DispString_EN(ptr->start_x + 8,
                        ptr->start_y + ((ptr->end_y - ptr->start_y - 16) / 2),
                        "REC");
...
/**
* @brief  Command_Recognize_Digit 执行手写数字识别
* @param  btn Touch_Button 类型的按键参数
* @retval 无
*/
static void Command_Recognize_Digit(void *btn)
